...
import by.shakhau.strpingsecurityjwt.controller.AuthenticationController.RefreshTokenRequest;
...
public class AuthenticationControllerTest {
    ...
    @Test
    public void shouldLoginUserAndGetSecuredDataAndLogout() throws Exception {
        ...
        // Try refresh token twice
        TokenResponse refreshToken = webClient.post().uri("/auth/refresh")
                .body(BodyInserters.fromValue(new RefreshTokenRequest(refToken)))
                .exchange()
                .expectStatus().isOk()
                .expectBody(TokenResponse.class)
                .returnResult().getResponseBody();
        ...
        refreshToken = webClient.post().uri("/auth/refresh")
                .body(BodyInserters.fromValue(new RefreshTokenRequest(refreshToken.getRefreshToken())))
                .exchange()
                .expectStatus().isOk()
                .expectBody(TokenResponse.class)
                .returnResult().getResponseBody();
